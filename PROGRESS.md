...
**2026-08-07 (continued)**: Started -- fix the globe seam (visible square box), make rain slightly more frequent, make ripples naturally interact, and dramatically increase live bead count to fill at least half the screen while optimizing for performance. Designed with Opus, implemented with Sonnet.

**2026-08-07 (continued)**: Done.

- **Globe seam fix**: the square box around the globe was three stacked discontinuities, not one -- (1) `useBackdropBase` clipped the brick photo out of the globe's whole square box instead of just its circular silhouette, leaving a brick-free square whose edge got doubled in contrast by this session's earlier `TEX_REVEAL_GAIN`; (2) the globe-composite plane rendered a static pre-baked crop while the water plane behind it now has an always-on ambient swell; (3) rain ripples were rejected from spawning in the whole square box, so they visibly clipped at its edge. Root fix: extracted the water shader into shared GLSL chunks (`WATER_UNIFORMS_GLSL`, `WATER_COMMON_GLSL` with a new `waterColor(p)` function) exported from WaterSurface.tsx, and gave the globe-composite plane its own shader that evaluates the SAME water field (over the same shared uniform objects, via a new `useWaterUniforms` hook) with the live globe composited on top in the fragment. The two planes now agree pixel-for-pixel with nothing to feather. Also narrowed the brick clip from a rect to a disc, and switched `pickSpawnPoint` to reject only the globe's circular disc instead of its whole square box.
- **Rain frequency**: `SPAWN_MEAN_MS` 340 -> 260 (~24% more drops); `MAX_RIPPLES` 10 -> 14 to keep headroom now that steady-state population rises to ~9.5.
- **Ripple interaction**: added one non-linear "crest steepening" step applied once to the FINAL summed gradient (never per-ripple) -- overlapping wavefronts now visibly spike brighter/steeper where they cross, and `SPEC_GATE` lowered 12 -> 6 so destructive interference reads as a genuine dark null instead of staying saturated.
- **Bead count**: replaced the flat `MAX_CAPACITY = 40` with a viewport-derived `capacityFor()` (~110 beads at 1280x800, targeting 50% screen coverage via 2D disc-packing math, hard-capped at 160), `marbleCount.ts`'s per-stream range widened to [10, 80], `SPAWN_JITTER_PX` 200 -> 420 and `BATCH_SPAWN_INTERVAL_MS` 120 -> 80 so a populous country's batch can actually reach and fill that ceiling.
- **Performance offsets** (explicit ask): `BEAD_GEOMETRY` 32x32 -> 16x16 segments (vertex load down ~27% net despite ~2.75x the beads), removed the scene's static `<directionalLight>` (one of two real-time light blocks per fragment) compensated by `BEAD_ENV_INTENSITY` 0.45 -> 0.52, `gl.transmissionResolutionScale` 0.5 -> 0.4, added `linearDamping`/`angularDamping` to beads so more of the pile sleeps (Rapier bodies asleep cost ~nothing). `BEAD_ENV_RESOLUTION` (256) deliberately left alone -- it's a one-time bake with zero per-frame cost, already at a hard floor per its own comment (BEAD_ROUGHNESS 0.05 needs the full 256 base mip).

Verification: `npm run build` clean, `npx oxlint src` clean (only the same class of pre-existing fast-refresh warnings already present elsewhere, e.g. GlobeRain.tsx). Live-verified via the same synthetic-click technique as prior rounds: bead scene mounted (2 canvases, United States/2022 panel), preview server logs show clean HMR updates with no transform/parse errors, and no Shader Error/WebGLProgram/GLSL console entries appeared, meaning both the water plane's and the new globe-composite plane's shaders compiled and linked. Actual visual verdict (does the seam actually read as gone, does the pile actually look "dramatic" and performant) still needs the user's own eyes -- this sandbox can't meaningfully screenshot WebGL content.

Status: done. Committing with the standing backdated timestamp.

**2026-08-07 (continued)**: Started/Done -- user reported the bead-lighting perf cut from the previous round made marbles read as flat, low-contrast discs and dulled the backdrop texture, and asked for circular (not elliptical) ripples. User rejected an Opus design dispatch for this and clarified the screenshot in their report was the PREVIOUS look they preferred, not the new one -- this is a direct revert + one constant removal, not a new design decision.

Reverted: restored `<directionalLight position={[200, 400, 300]} intensity={0.4} />` in BeadScene.tsx's Canvas (removed last round for a bead-count perf cut) and reverted `BEAD_ENV_INTENSITY` 0.52 -> 0.45 (the compensation for the removed light, no longer needed). The baked Lightformer rig supplies IBL but not a real-time Lambert shading gradient with a fixed screen direction -- without the light, beads lost the visible shading sweep that read as glass rather than flat discs, which is also what made the backdrop texture read as comparatively less visible (lower overall scene contrast). Updated the surrounding comments (MOUSE_LIGHT_INTENSITY's scale-reference comment, the Canvas JSX comment) back to match.

Removed the ripple anisotropy added last round (`ANISO` constant and the per-ripple stretch-matrix/axis-hash math in WaterSurface.tsx's `waterColor()`) per explicit request for circular ripples -- simplified the loop back to a plain circular band test and radial gradient, while keeping the per-ripple speed variance and wavelength jitter (unrelated to shape, and not what was asked about).

Verification: `npm run build` clean, `npx oxlint src` clean (same pre-existing warning class). Live-verified: bead scene mounted (2 canvases, via a synthetic click on the Paris label), no Shader Error/GLSL console entries -- both shaders compiled and linked with the restored light and simplified ripple code. Visual verdict is the user's own call on this round, per their explicit report.

Status: done. Committing with the standing backdated timestamp.

**2026-08-07 (continued)**: Started/Done -- user reported (with screenshot) that dark-theme marbles were still not dark enough after last round's directional-light restore ("ideally, they should be pitch black"). Root cause: BEAD_TRANSMISSION=0.98 means the glass body should mostly show the (dark) scene behind it, but the ambient/reflection contribution (BeadEnvironment's baked Lightformer rig, 0.35 intensity in dark theme, times BEAD_ENV_INTENSITY) plus the restored directionalLight's full-strength 0.4 Lambert term were both tuned/reused as-is from the light theme, adding enough uniform brightness across each bead's lit hemisphere to read as grey rather than near-black glass.

Fix (direct tuning, no Opus dispatch -- straightforward parameter adjustment, not a new design decision): made both lights theme-conditional, lowered only for dark theme. BeadEnvironment's dark-theme `intensity` 0.35 -> 0.16; the Canvas's `<directionalLight>` intensity `theme === 'dark' ? 0.15 : 0.4` (was a flat 0.4 for both themes). Light theme values are untouched, since that theme's marbles were not part of the report and the near-white light-theme backdrop needs the stronger light already tuned for it. Both lights are still present in dark theme, just dimmer -- keeps the shading gradient/shape cue from last round's restore while letting BEAD_TRANSMISSION's dark background show through instead of being washed out.
...
**2026-08-07 (continued)**: Started -- four new bead-scene effects, designed with Opus (extensive: read three@0.185.1's actual shader chunk source for Effect A), implemented with Sonnet:

A. Cursor-lit warm refraction on birth beads: `onBeforeCompile` patch (`patchWarmRefraction` in BeadScene.tsx) injected into `<transmission_fragment>`, multiplying `material.diffuseContribution` (what three's transmission chunk multiplies the refracted background sample by) by a proximity-gated mix toward a hue-normalized `--accent` tint. Verified against actual three source: the injection point runs AFTER all lighting chunks have consumed `diffuseContribution`, so it recolors only the refraction. One shared `onBeforeCompile` function reference across all birth materials (load-bearing for `WebGLPrograms.getProgramCacheKey`, which hashes `onBeforeCompile.toString()`, not shader source -- a fresh closure per material would still work but is fragile). Death beads untouched.
B. Brick backdrop texture now genuinely cursor-gated instead of permanently visible: `BACKDROP_TEXTURE_OPACITY` 0.2 -> 0.06 (near-invisible baseline), `WaterSurface.tsx`'s `TEX_REVEAL_GAIN` 1.1 -> 4.6 (the shader-side cursor-local amplifier already existed from an earlier round, just wasn't carrying the whole visibility burden yet).
C. Drastically more rain: `SPAWN_MEAN_MS` 260 -> 75 (steady-state population ~9.5 -> ~31.7 live), `MAX_RIPPLES` 14 -> 44, `SPAWN_MIN_MS` 90 -> 35 (had to move with the mean or the Poisson process would collapse into a metronome at high density).
D. Ripple thickness usually thin, sometimes random: replaced the flat `PACKET_PX = 90` with a per-ripple cubic-skewed envelope width (`PACKET_MIN_PX=26`/`PACKET_MAX_PX=130`, median ~37px, mean ~52px), hashed off the same two ripple-slot fields already used for wavelength jitter (a second independent hash, not reusing the first -- reusing it would have correlated thin ripples with a fixed wavelength bias). Required retuning `CREST_SOLO`/`CREST_CROSS` (0.26/0.48 -> 0.32/0.55): thinner envelopes have a steeper peak gradient, and C's much higher population raises the average simultaneous-crossing count, both of which would have made the "crest steepening" effect fire almost constantly instead of on genuine crossings.

Verification: `npm run build` clean, `npx oxlint src` clean (same pre-existing warning class). Live-verified: bead scene mounted (2 canvases, synthetic click on Dubai), no Shader Error/GLSL console entries -- confirming the onBeforeCompile string-injection patch (the riskiest piece, since it depends on exact chunk-name anchors matching the installed three version) actually compiled and linked. Actual visual read (does the red hue show, does the texture reveal feel right, does the ripple density/thickness variety look good) is the user's own call, same caveat as every WebGL round this session.

Status: done, proceeding to the explicit follow-up ask (aggressive performance plan with Opus).

**2026-08-07 (continued)**: Done -- aggressive performance pass on the bead scene, designed with Opus (detailed fragment-cost accounting across the water shader and bead rendering, explicit "aggressive, not surgical" framing per the user's request), implemented with Sonnet. Applied the plan's concrete, bounded items (skipped the riskiest one, InstancedRigidBodies, for a future dedicated pass -- took its safer fallback instead):

1. Globe-composite texture now uploads cobe's `<canvas>` element directly (`new THREE.Texture(globeElement)`, `generateMipmaps=false`) instead of round-tripping through an intermediate 2D canvas (`ctx.drawImage` GPU->CPU readback + re-upload + full mip-chain rebuild) -- the file's own comment already called that readback "the single most expensive per-frame op in the scene." `BACKDROP_UPDATE_EVERY_N_FRAMES` 2 -> 3 now that the per-update cost dropped.
2. Ripple population traded duration for count at an unchanged arrival rate: `RIPPLE_LIFE_S` 2.6 -> 1.3, `MAX_RIPPLES` 44 -> 26 (population ~31.7 -> ~15.9 live, same 12.2 drops/second "drastically more rain" from last round). Caught and fixed a rate bug in the delivered plan for a related change: it proposed doubling `BATCH_SPAWN_INTERVAL_MS` for the merged spawn timer below, which would have silently halved each stream's actual bead spawn rate rather than just halving the commit count -- kept the interval unchanged instead.
3. Merged the two independent-phase bead spawn `setInterval`s into one paired timer at the same interval, halving React commit frequency during a batch drain for an identical per-stream drain rate.
4. `MARBLE_VARIANTS` 4 -> 1 (useBeadMaterials' own comment already noted every variant was visually identical) -- 8 materials/sort-buckets/uniform-rebinds per frame down to 2.
5. Explicit `<BallCollider args={[BEAD_RADIUS]} />` instead of `colliders="ball"`, skipping Rapier's per-body-creation geometry inspection.
6. Capacity target cut (`TARGET_PILE_VIEWPORT_FRACTION` 0.5 -> 0.38, `MAX_CAPACITY_CEILING` 160 -> 110) as the safer fallback to the plan's preferred-but-unimplemented `InstancedRigidBodies` conversion (110+ individual mesh draw calls is a real CPU cost no shader tuning touches; instancing would cut it to ~2 draws and make the original target affordable again -- left as a flagged follow-up, not attempted this pass given its scope as a physics/eviction refactor).
7. Globe-composite shader's opaque early-out widened 0.999 -> 0.985 alpha (skips the full water-field evaluation across more of cobe's glow ring, below 8-bit quantization).
8. `gl.transmissionResolutionScale` 0.4 -> 0.34.

Explicitly NOT touched, per the plan's own reasoning: `patchWarmRefraction` (0.4% of scene cost, a branch would be net-negative), `BEAD_ENV_RESOLUTION` (baked once, zero per-frame cost regardless of resolution), `dpr={1}` (already floor), the two real-time lights (3% of GPU, needed for the glass-gradient look restored earlier this session), and a half-resolution water render target (would blur the exact dot/brick grain the recent cursor-reveal work depends on).
...
**2026-08-08**: Started/Done -- user asked for two related changes to reduce perceived lag and improve the marble-drop transition, designed with Opus (explicit timeline math for the tile-flip cycle, ~2.03s forward), implemented with Sonnet:

1. **Gate marble spawning on the tile transition actually concluding.** Previously `App.tsx` mounted `BeadScene` and triggered `TileTransition` in the same commit, and `BeadScene`'s spawn timer started immediately on mount -- meaning marble spawning (React commits, Rapier body creation, glass draw calls) was already running while the CSS 3D tile-flip animation was still mid-flight, competing for the same frame budget behind an opaque overlay. Added an `onSettled` callback prop to `TileTransition` (fires once, exactly when `phase` returns to `'idle'` -- confirmed via the actual timeline math that this is the only instant the overlay has genuinely stopped rendering, not `'revealing'` which is ~1.2s earlier and just starts the flip). `App.tsx` derives `beadsReady = beadSceneVisible && tilesSettled` and passes it to `BeadScene` as a new `spawnEnabled` prop -- gating only the spawn timer, not the component mount (which still must land in the same commit per `LEAD_IN_FORWARD_MS`'s own existing rationale, so the WebGL canvas still gets its full init window). Handled the year-dropdown edge case explicitly: `tilesSettled` is reset via a render-time (not effect-time) comparison against `beadSceneVisible`, not against `BeadScene`'s own remount key, since changing only the year re-mounts `BeadScene` without re-triggering a tile-transition cycle -- a component-local flag would have gotten permanently stuck false on a year switch.
2. **Eased, longer spawn pacing.** Replaced the flat `BATCH_SPAWN_INTERVAL_MS = 80` `setInterval` with a chained `setTimeout` whose gap grows across the batch (`gap(u) = 88 + (240-88) * u^2`, fast at the start, decelerating) -- deliberately parameterized as gap-as-a-function-of-progress rather than the more obvious time-as-a-function-of-index ease-out formula, since the latter's instantaneous start-of-batch rate is a multiple of its mean rate and would have made the busiest moment of the new schedule *busier* than the flat rate it replaces. The new 88ms floor is strictly above the old flat 80ms, so peak commit rate actually drops (12.5/s -> 11.4/s) alongside the mean (7.3/s). A full 80+80 batch now takes ~11s (was ~6.4s) plus the new ~2s tile-transition gate. Kept the two streams (births/deaths) on ONE shared timeline sized by the larger count, rather than each easing independently -- an independent-timeline design was evaluated and rejected: it would have desynced the two `YearCounters` (a lopsided country's smaller stream would finish and freeze early) and, worse for the actual ask, doubled the peak commit rate back above the pre-merge baseline the last performance pass fixed.
...
**2026-08-08 (continued)**: Started/Done -- bug report: shrinking the browser window while marbles were present left them stuck in the frame. Systematic-debugging, root cause found by inspection (reproducible logic bug, not a flaky/environmental issue) rather than a guessed fix.

Root cause: `Boundaries` (BeadScene.tsx, the floor/side-wall/front-back-pin RigidBodies) recomputes its `position`/`CuboidCollider args` from `useThree`'s `width`/`height` on every resize, but never remounts -- and `@react-three/rapier` only reads a fixed RigidBody's `position` and collider args ONCE, at creation. The file's own `GlobeCollider` component right below it already documents this exact Rapier behavior and works around it with a `key` tied to its own position/size, forcing a clean unmount/remount on every resize -- `Boundaries` was simply missing the same treatment. Without it, shrinking the window re-rendered the wall elements with new (smaller) numbers that Rapier silently ignored: the actual physics walls stayed pinned at their original, larger creation-time size and position, no longer lined up with the now-smaller visible viewport, so beads piled up against invisible walls in the wrong place relative to what's on screen.

Fix: added a `key={`${sizeKey}:<wall-name>`}` (sizeKey = rounded width:height) to each of the five wall RigidBodies, exactly mirroring GlobeCollider's existing pattern, so a resize now fully recreates all five walls at the correct new position/size instead of leaving stale colliders behind.

Verification: `npm run build` clean, `npx oxlint src` clean (same pre-existing warning class). Live-verified: selected a country (2 canvases mounted), resized the viewport from 1280x800 down to 700x500 with beads live, no new console errors (Rapier throws on a failed collider recreation, and none appeared) and both canvases remained mounted afterward. This sandbox cannot directly observe live bead positions in the physics world or take a screenshot to visually confirm beads no longer clip through/stick on the new wall boundary -- that visual confirmation across a few resize sizes is the user's own call, though the fix targets the exact mechanism (stale-collider-after-resize) the codebase already names as the cause of this class of bug in the adjacent `GlobeCollider` component.
